...
# to beginning the network connection check
 ...
 ScriptLogging "$install_dir present on Mac. Checking for available Apple software updates."
 
 sleep 10
 
  # Once the network connection is available and the script has verified
  # that the install directory is available, the script will check for
  # available Apple software updates and install them. Once updates have
  # been found and installed, the Mac will reboot. On reboot, the check will
  # re-occur until all available Apple software updates have been installed.
  
  # The AvailableUpdatesNoRestartNeeded and AvailableUpdatesRestartRequired variables 
  # are being declared at this point in the script because when they are declared at 
  # the beginning of the script, the softwareupdate check is run immediately after 
  # the script's launch. Running the softwareupdate check will delay all other processes 
  # in the script, including the command to unload the loginwindow launchdaemon.
  # 
  # In a previous version of this script, having the softwareupdate check delay the unload
  # of the loginwindow LaunchDaemon resulted in the loginwindow loading and then being
  # unloaded and reloaded a second time. This is undesirable behavior (and confusing to
  # look at), so those variables are now down here.
  ...
  if [[ "$AvailableUpdatesRestartRequired" != "" ]] || [[ "$AvailableUpdatesNoRestartNeeded" != "" ]]; then
    ...
    ScriptLogging "Apple software updates have been installed. Restarting Mac."
  ...
  if [[ "$AvailableUpdatesRestartRequired" == "" ]] && [[ "$AvailableUpdatesNoRestartNeeded" == "" ]]; then
    ScriptLogging "All available Apple software updates have been installed."
  fi
